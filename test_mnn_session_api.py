...
input_data1 = MNN.Tensor((1, dst_height, dst_width, channel),
...
image = cv.imread(img_path)
image = cv.resize(image, (224, 224), mean=[
                  103.94, 116.78, 123.68], norm=[0.017, 0.017, 0.017])
input_data2 = np.expand_dims(image, 0)  # [224, 224, 3] -> [1, 224, 224, 3]

# 合并2张图片到，并赋值给input_data
input_data1 = expr.const(input_data1.getHost(
), input_data1.getShape(), expr.NHWC)  # Tensor -> Var
...
# 从输出Tensor拷贝出数据
output_data = MNN.Tensor(output_tensor.getShape(
), MNN.Halide_Type_Float, MNN.Tensor_DimensionType_Caffe)
